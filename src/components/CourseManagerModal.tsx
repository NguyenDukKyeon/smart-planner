...
export function CourseManagerModal({ currentSubjects, onSubjectsUpdated, progress, activeTimerLessonId, trigger }: Props) {
  ...
  const minutesByLesson = useMemo(() => {
    ...
    for (const session of progress?.studySessions ?? []) {
      map.set(session.lessonId, (map.get(session.lessonId) ?? 0) + Math.round(session.durationSeconds / 60));
    }
    return map;
  ...
  const apply = (
    ...
    const ok =
      result == null ? true : typeof result === "boolean" ? result : result.ok;
    if (!ok) return false;
  ...
  const createSubject = () => {
    ...
    if (currentSubjects.some((subject) => subject.name.localeCompare(name, "vi", { sensitivity: "base" }) === 0)) {
      ...
      .map((milestone) => ({
        ...
        lessons: milestone.lessons
          ...
          .sort((a, b) => {
            ...
            if (sort === "remaining") {
              return Math.max(0, a.plannedDurationMinutes - aMinutes) - Math.max(0, b.plannedDurationMinutes - bMinutes);
            }
            return 0;
          }),
      }))
  ...
  const subjectStats = (subject: Subject) => {
    ...
    const remaining = lessons.reduce(
      (sum, lesson) => sum + Math.max(0, lesson.plannedDurationMinutes - (minutesByLesson.get(lesson.id) ?? 0)),
      0,
    );
    return { lessons: lessons.length, completed, remaining, percent: lessons.length ? Math.round((completed / lessons.length) * 100) : 0 };
  };
  ...
  const saveTopic = () => {
    ...
    if (
      apply(
        next,
        topicEditor.id ? `Đã đổi tên chủ đề thành ${title}.` : `Đã thêm chủ đề ${title}.`,
      )
    ) {
      setTopicEditor(null);
    }
  ...
  return (
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {trigger ?? (
          <Button variant="outline" size="sm" className="h-9 gap-1.5 rounded-2xl border-indigo-200 bg-indigo-50/70 text-xs font-semibold text-indigo-800">
      ...
      <DialogContent className="h-[94vh] w-[97vw] max-w-6xl overflow-hidden rounded-3xl p-0 grid-rows-[auto_minmax(0,1fr)]">
        <DialogHeader className="border-b bg-white px-5 py-4">
          ...
          <DialogDescription>Tổ chức môn học, chủ đề và các bài trong lộ trình của bạn.</DialogDescription>
        </DialogHeader>

        <div className="grid min-h-0 flex-1 md:grid-cols-[280px_1fr]">
          <aside className={cn("min-h-0 overflow-y-auto border-r bg-slate-50/80 p-3", mobileDetail && "hidden md:block")}>
            <div className="rounded-2xl border bg-white p-3">
              <div className="grid grid-cols-[64px_1fr] gap-2">
                <Input value={newSubjectEmoji} maxLength={4} onChange={(event) => setNewSubjectEmoji(event.target.value)} className="text-center text-lg" aria-label="Biểu tượng môn mới" />
                <Input value={newSubjectName} onChange={(event) => setNewSubjectName(event.target.value)} onKeyDown={(event) => event.key === "Enter" && createSubject()} placeholder="Tên môn học mới" />
              </div>
              <Button type="button" className="mt-2 w-full rounded-xl bg-indigo-600 hover:bg-indigo-700" onClick={createSubject}>
            ...
            <div className="relative mt-3">
              ...
              <Input value={subjectSearch} onChange={(event) => setSubjectSearch(event.target.value)} placeholder="Tìm môn học…" className="rounded-xl bg-white pl-9" />
            </div>

            <Tabs value={archiveView ? "archived" : "active"} onValueChange={(value) => setArchiveView(value === "archived")} className="mt-3">
              <TabsList className="grid w-full grid-cols-2 rounded-xl">
                <TabsTrigger value="active" className="rounded-lg text-xs">Đang học</TabsTrigger>
                <TabsTrigger value="archived" className="rounded-lg text-xs">Đã lưu trữ</TabsTrigger>
              </TabsList>
            </Tabs>
            ...
            {!archiveView ? (
              <div className="mt-3 space-y-2">
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        selectedSubjectId === subject.id ? "border-indigo-300 bg-indigo-50" : "border-slate-200 bg-white hover:bg-slate-50",
                      )}
                    >
                      <div className="flex items-start gap-2">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-[11px] text-slate-500">{stats.completed} / {stats.lessons} bài · {formatMinutes(stats.remaining)} còn lại</p>
                ...
                {currentSubjects.length === 0 ? (
                  <EmptyState title="Bạn chưa có môn học nào" description="Tạo môn đầu tiên hoặc sử dụng lộ trình mẫu lớp 11." />
              ...
              <div className="mt-3 space-y-2">
                ...
                {archived.subjects.length === 0 && archived.lessons.length === 0 && (
                  <EmptyState title="Kho lưu trữ đang trống" description="Môn và bài được lưu trữ sẽ xuất hiện tại đây." />
                )}
              </div>
            )}
            ...
            <Button
              ...
              onClick={() => {
                ...
                if (restored) apply(restored, "Đã hoàn tác thay đổi danh mục gần nhất.", { alreadyPersisted: true });
          ...
          <main className={cn("min-h-0 overflow-y-auto bg-white p-4 sm:p-5", !mobileDetail && "hidden md:block")}>
            {selectedSubject ? (
              <>
                <Button type="button" variant="ghost" size="sm" className="mb-3 rounded-xl md:hidden" onClick={() => setMobileDetail(false)}>
                ...
                <SubjectHeader
                  ...
                  onArchive={() => {
                    ...
                    if (apply(archiveSubject(currentSubjects, selectedSubject.id), `Đã lưu trữ môn ${selectedSubject.name}.`, { alreadyPersisted: true })) {
                      setArchiveVersion((version) => version + 1);
                    }
                  }}
                  onDelete={() => {
                    const lessons = allLessons(selectedSubject);
                    if (!confirmTimerImpact(lessons.map((lesson) => lesson.id), `xóa môn ${selectedSubject.name}`)) return;
                    setPendingDelete({
                  ...
                  addLesson={
                    <AddLessonModal
                      ...
                      trigger={<Button className="rounded-xl bg-emerald-600 hover:bg-emerald-700"><Plus className="h-4 w-4" /> Thêm bài học</Button>}
                    />
                  }
                />
                ...
                <div className="mt-5 grid gap-3 lg:grid-cols-[minmax(220px,1fr)_170px_170px_auto]">
                  <div className="relative">
                    <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                    <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Tìm tên bài hoặc chủ đề…" className="rounded-xl pl-9" />
                  </div>
                  <select value={filter} onChange={(event) => setFilter(event.target.value as LessonFilter)} className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm">
                  ...
                  <select value={sort} onChange={(event) => setSort(event.target.value as LessonSort)} className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm">
                ...
                {selectionMode && (
                  <section className="mt-3 rounded-2xl border border-indigo-200 bg-indigo-50/60 p-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <p className="text-sm font-semibold text-indigo-950">Đã chọn {selectedLessonIds.size} bài học</p>
                      <Button
                        type="button"
                        size="sm"
                        variant="outline"
                    ...
                    <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-5">
                      <div className="flex gap-2">
                        <select value={bulkTargetSubjectId} onChange={(event) => setBulkTargetSubjectId(event.target.value)} className="h-9 min-w-0 flex-1 rounded-xl border border-indigo-200 bg-white px-2 text-xs">
                          <option value="">Chuyển sang môn…</option>
                          {currentSubjects.filter((subject) => subject.id !== selectedSubject.id).map((subject) => <option key={subject.id} value={subject.id}>{subject.name}</option>)}
                        </select>
                        <Button type="button" size="sm" variant="outline" className="rounded-xl" disabled={!bulkTargetSubjectId || selectedLessonIds.size === 0} onClick={() => applyBulk(moveLessonsToSubject(currentSubjects, selectedLessonIds, bulkTargetSubjectId), `Đã chuyển ${selectedLessonIds.size} bài học.`)} aria-label="Chuyển các bài đã chọn"><ChevronRight className="h-4 w-4" /></Button>
                      </div>
                      <div className="flex gap-2">
                        <select value={bulkTargetTopicId} onChange={(event) => setBulkTargetTopicId(event.target.value)} className="h-9 min-w-0 flex-1 rounded-xl border border-indigo-200 bg-white px-2 text-xs">
                          <option value="">Chuyển sang chủ đề…</option>
                          {selectedSubject.milestones.map((milestone) => <option key={milestone.id} value={milestone.id}>{milestone.title}</option>)}
                        </select>
                        <Button type="button" size="sm" variant="outline" className="rounded-xl" disabled={!bulkTargetTopicId || selectedLessonIds.size === 0} onClick={() => applyBulk(moveLessonsToTopic(currentSubjects, selectedLessonIds, selectedSubject.id, bulkTargetTopicId), `Đã chuyển ${selectedLessonIds.size} bài sang chủ đề mới.`)} aria-label="Chuyển các bài sang chủ đề"><ChevronRight className="h-4 w-4" /></Button>
                      </div>
                      <div className="flex gap-2">
                        <Input type="date" value={bulkDate} onChange={(event) => setBulkDate(event.target.value)} className="h-9 min-w-0 flex-1 rounded-xl bg-white text-xs" />
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button type="button" size="sm" variant="outline" className="rounded-xl" disabled={selectedLessonIds.size === 0} onClick={() => applyBulk(updateLessonsDetails(currentSubjects, selectedLessonIds, { plannedDurationMinutes: bulkMinutes }), `Đã đặt mục tiêu ${bulkMinutes} phút cho ${selectedLessonIds.size} bài.`)}>Áp dụng</Button>
                      </div>
                      <div className="flex gap-2">
                        <Button type="button" size="sm" variant="outline" className="flex-1 rounded-xl" disabled={selectedLessonIds.size === 0} onClick={() => { if (confirmTimerImpact(selectedLessonIds, `lưu trữ ${selectedLessonIds.size} bài học`) && window.confirm(`Lưu trữ ${selectedLessonIds.size} bài học? Lịch sử phiên học vẫn được giữ.`)) applyBulk(archiveLessons(currentSubjects, selectedLessonIds), `Đã lưu trữ ${selectedLessonIds.size} bài học.`, true); }}><Archive className="h-4 w-4" /> Lưu trữ</Button>
                        <Button type="button" size="sm" variant="outline" className="rounded-xl text-red-700" disabled={selectedLessonIds.size === 0} onClick={() => {
                          ...
                          setPendingDelete({
                            ...
                            description: "Các bài sẽ bị xóa khỏi lộ trình và lịch tương lai. Lịch sử phiên học vẫn được giữ.",
                        ...
                        }} aria-label="Xóa các bài đã chọn"><Trash2 className="h-4 w-4" /></Button>
                      </div>
                    </div>
                  </section>
                )}
                ...
                <div className="mt-4 space-y-3">
                  {filteredMilestones.map((milestone) => (
                    <TopicGroup
                      ...
                      onEditTopic={() => setTopicEditor({ id: milestone.id, title: milestone.title })}
                      ...
                      onDeleteTopic={() => {
                        setPendingDelete({
                          ...
                          nextSubjects: removeTopicAndMoveLessonsToUncategorized(currentSubjects, selectedSubject.id, milestone.id),
                      ...
                      onRequestDelete={(lesson) => {
                        ...
                        setPendingDelete({
                          ...
                          description: "Bài sẽ bị xóa khỏi lộ trình và lịch tương lai. Lịch sử phiên học đã diễn ra vẫn được giữ.",
                  ...
                  {filteredMilestones.length === 0 && (
                    <EmptyState title="Không tìm thấy bài học phù hợp" description="Hãy thử từ khóa hoặc bộ lọc khác." />
                  )}
                </div>
              </>
            ) : (
              <EmptyState title="Chọn một môn học" description="Chọn môn ở cột bên trái để xem và tổ chức các bài học." />
            )}
          </main>
        </div>
      </DialogContent>
      ...
      <Dialog open={Boolean(topicEditor)} onOpenChange={(next) => !next && setTopicEditor(null)}>
        <DialogContent className="max-w-md rounded-3xl">
          <DialogHeader>
            ...
            <DialogDescription>Bài học có thể được thêm vào chủ đề này ngay hoặc sau đó.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                onChange={(event) => setTopicEditor((current) => (current ? { ...current, title: event.target.value } : current))}
                onKeyDown={(event) => event.key === "Enter" && saveTopic()}
                placeholder="VD: Chương 2: Dãy số"
              />
            </div>
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => setTopicEditor(null)}>Hủy</Button>
              <Button onClick={saveTopic}>{topicEditor?.id ? "Lưu thay đổi" : "Thêm chủ đề"}</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={Boolean(editingSubject)} onOpenChange={(next) => !next && setEditingSubject(null)}>
        <DialogContent className="max-w-md rounded-3xl">
          <DialogHeader><DialogTitle>Chỉnh sửa môn học</DialogTitle><DialogDescription>Cập nhật tên và biểu tượng môn.</DialogDescription></DialogHeader>
          <div className="space-y-4">
            <div><Label>Tên môn học</Label><Input value={subjectDraft.name} onChange={(event) => setSubjectDraft((current) => ({ ...current, name: event.target.value }))} /></div>
            <div><Label>Biểu tượng</Label><Input value={subjectDraft.emoji} maxLength={4} onChange={(event) => setSubjectDraft((current) => ({ ...current, emoji: event.target.value }))} /></div>
            <div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setEditingSubject(null)}>Hủy</Button><Button onClick={saveSubject}>Lưu thay đổi</Button></div>
          </div>
        </DialogContent>
      </Dialog>
  ...
  return (
    <section className="rounded-3xl border bg-gradient-to-br from-white to-indigo-50/60 p-5 shadow-xs">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
        <span className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-white text-3xl shadow-xs">{subject.emoji}</span>
        <div className="min-w-0 flex-1">
          <h2 className="font-serif text-2xl font-semibold text-slate-900">{subject.name}</h2>
          <p className="mt-1 text-sm text-slate-500">{subject.milestones.length} chủ đề · {stats.lessons} bài · {stats.completed} đã hoàn thành</p>
          <div className="mt-3 flex items-center gap-3"><Progress value={stats.percent} className="h-2 flex-1" /><span className="text-xs font-bold text-indigo-700">{stats.percent}%</span></div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {addLesson}
          <DropdownMenu>
            <DropdownMenuTrigger asChild><Button variant="outline" size="icon" className="rounded-xl" aria-label="Quản lý môn"><MoreHorizontal className="h-4 w-4" /></Button></DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="rounded-xl">
              <DropdownMenuItem onSelect={onEdit}><Edit3 className="mr-2 h-4 w-4" /> Chỉnh sửa môn</DropdownMenuItem>
              <DropdownMenuItem onSelect={onExport}><Download className="mr-2 h-4 w-4" /> Xuất riêng môn này</DropdownMenuItem>
              <DropdownMenuItem disabled={!canMoveUp} onSelect={onMoveUp}><ArrowUp className="mr-2 h-4 w-4" /> Di chuyển lên</DropdownMenuItem>
              <DropdownMenuItem disabled={!canMoveDown} onSelect={onMoveDown}><ArrowDown className="mr-2 h-4 w-4" /> Di chuyển xuống</DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem onSelect={onArchive}><Archive className="mr-2 h-4 w-4" /> Lưu trữ môn</DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem onSelect={onDelete} className="text-red-700"><Trash2 className="mr-2 h-4 w-4" /> Xóa môn và các bài</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </div>
        ...
        <div className="flex w-full items-center gap-2 bg-slate-50/80 px-3 py-2.5">
          <CollapsibleTrigger asChild>
            <button type="button" className="flex min-w-0 flex-1 items-center gap-3 text-left">
              ...
              <div className="min-w-0 flex-1"><h3 className="truncate text-sm font-semibold text-slate-900">{title}</h3><p className="text-[11px] text-slate-500">{completed} / {lessons.length} bài · {formatMinutes(remaining)} còn lại</p></div>
            </button>
          </CollapsibleTrigger>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button type="button" variant="ghost" size="icon" className="h-8 w-8 rounded-lg" aria-label={`Quản lý chủ đề ${title}`}>
                <MoreHorizontal className="h-4 w-4" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="rounded-xl">
              <DropdownMenuItem onSelect={onEditTopic}><Edit3 className="mr-2 h-4 w-4" /> Đổi tên chủ đề</DropdownMenuItem>
              <DropdownMenuItem disabled={!canMoveTopicUp} onSelect={onMoveTopicUp}><ArrowUp className="mr-2 h-4 w-4" /> Di chuyển lên</DropdownMenuItem>
              <DropdownMenuItem disabled={!canMoveTopicDown} onSelect={onMoveTopicDown}><ArrowDown className="mr-2 h-4 w-4" /> Di chuyển xuống</DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem onSelect={onDeleteTopic} className="text-red-700"><Trash2 className="mr-2 h-4 w-4" /> Xóa chủ đề</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
        <CollapsibleContent>
          {lessons.length === 0 ? (
            <p className="px-4 py-5 text-center text-xs text-slate-500">Chủ đề này chưa có bài học.</p>
          ) : (
                ...
                >
                  ...
                  {canReorder && !selectionMode && (
                    <div className="flex shrink-0 items-center gap-1" aria-label={`Sắp xếp ${lesson.title}`}>
                      <Button
                        ...
                        disabled={index === 0}
                        onClick={() => onApply(reorderLesson(subjects, lesson.id, -1), "Đã di chuyển bài học lên.")}
                      ...
                      <Button
                        ...
                        onClick={() => onApply(reorderLesson(subjects, lesson.id, 1), "Đã di chuyển bài học xuống.")}
                  ...
                  {!selectionMode && <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-56 rounded-xl">
                      <DropdownMenuItem onSelect={() => onEdit(lesson)}><Edit3 className="mr-2 h-4 w-4" /> Chỉnh sửa bài học</DropdownMenuItem>
                      {subjects.filter((candidate) => candidate.id !== subject.id).map((candidate) => (
                        <DropdownMenuItem key={candidate.id} onSelect={() => onApply(moveLessonToSubject(subjects, lesson.id, candidate.id), `Đã chuyển bài sang ${candidate.name}.`)}>
                          <BookOpen className="mr-2 h-4 w-4" /> Chuyển sang {candidate.name}
                        </DropdownMenuItem>
                      ))}
                      <DropdownMenuItem onSelect={() => onApply(duplicateLessonInSubjects(subjects, lesson.id), `Đã nhân bản bài ${lesson.title}.`)}><Plus className="mr-2 h-4 w-4" /> Nhân bản bài học</DropdownMenuItem>
                      <DropdownMenuSeparator />
                      <DropdownMenuItem onSelect={() => {
                        if (!confirmTimerImpact([lesson.id], `lưu trữ bài ${lesson.title}`)) return;
                        if (onApply(archiveLesson(subjects, lesson.id), `Đã lưu trữ bài ${lesson.title}.`, { alreadyPersisted: true })) {
                          onArchiveChanged();
                        }
                      }}><Archive className="mr-2 h-4 w-4" /> Lưu trữ</DropdownMenuItem>
                      <DropdownMenuItem className="text-red-700" onSelect={() => onRequestDelete(lesson)}><Trash2 className="mr-2 h-4 w-4" /> Xóa bài học</DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>}
                </li>
              );
...
function ArchivedItem({ label, onRestore }: { label: string; onRestore: () => void }) {
  return <div className="flex items-center gap-2 rounded-xl border bg-white p-2"><FolderArchive className="h-4 w-4 text-amber-600" /><span className="min-w-0 flex-1 truncate text-xs font-medium">{label}</span><Button size="icon" variant="ghost" className="h-8 w-8 rounded-lg" onClick={onRestore} aria-label={`Khôi phục ${label}`}><Undo2 className="h-4 w-4" /></Button></div>;
}

function EmptyState({ title, description }: { title: string; description: string }) {
  return <div className="rounded-2xl border border-dashed bg-slate-50 p-7 text-center"><LibraryBig className="mx-auto h-8 w-8 text-slate-300" /><h3 className="mt-2 text-sm font-semibold text-slate-800">{title}</h3><p className="mt-1 text-xs text-slate-500">{description}</p></div>;
}
