# P1E-HF1 Forecast Correctness Completion Evidence

**Repository:** `NguyenDukKyeon/smart-planner`  
**Package:** `P1E-HF1 Forecast Correctness`  
**Date:** 2026-08-08  
**PR:** #10  
**Branch:** `fix/p1e-hf1-forecast-correctness`  
**Exact predecessor:** `main@78f042255fe46e9bbc69193e6ef47158442cdf03`  
**Literal final source/test head:** `a6326d13b438d6cc93c59925740b293711f802fb`  
**Disposition at refresh:** `IMPLEMENTED / REVIEW_PENDING / NOT_MERGED`

---

## 1. Authority and real-world reproduction

Design spec:

- initial: `2673b1c5ea2b7d0a47cceb57c262ae3911e46258`
- final self-reviewed: `df59a042087d51e6cbb0f0cedd3b4fa642f77483`

Implementation plan:

- initial: `ddd09c74425cb868772f9ae431b3edc07197476b`
- final self-reviewed: `80e3b884553990059af770fbaef52e020b2c2dfd`

The user-provided `lo_trinh_hien_tai.csv` was reproduction evidence only and was not committed. Its verified shape is encoded by a deterministic synthetic regression fixture:

```text
352 unique lessons
160 Toán
116 Hóa học
69 Vật lý
7 Tiếng Anh
345 × 120 minutes
6 × 90 minutes
1 × 30 minutes
41,970 total planned minutes = 699.5 hours
```

After eleven deterministic 120-minute completions:

```text
341 lessons remain
40,650 planned minutes remain = 677.5 hours
```

The former production value near `210.3` new-learning hours for 341 remaining lessons was therefore incompatible with the current catalog itself.

---

## 2. Task 1 — lesson-level study-duration evidence

### Valid RED

```text
head       eaa17253fc81171c9228f5c91da9612dd7132a0b
workflow   Build diagnostics #328
run        31250481356
job        93085973579
merge ref  413b3a89954ea36cbdbc2abc599c474ba3e5bea1
```

Typecheck/lint PASS. 57/58 files and 360/363 tests passed. The three intended failures required the missing lesson-level evidence selector.

### Invalid intermediate

Build diagnostics #329 was blocked by one Prettier error before tests and is not GREEN evidence.

### Valid GREEN

```text
head       6af7c815d2c5d4562fd06ab84db8b9745a72483b
workflow   Build diagnostics #330
run        31250666866
job        93086431610
merge ref  7a707c4a38a3aa90d760f2c1b7965ad8bc455e57
```

58/58 files, 363/363 tests, typecheck/lint/build/clean-tree PASS.

Locked behavior:

- repeated study sessions on one completed lesson become one evidence lesson;
- non-review `focus-timer` and `manual` sessions are accepted;
- review sessions are excluded;
- incomplete/deleted-catalog lessons are excluded from confidence evidence;
- confidence counts evidence lessons rather than raw sessions.

---

## 3. Task 2 — planned lesson cost and canonical daily capacity

### Valid RED

```text
head       b2d57fdafaddba143969ed18aa7cf3769af7d9d8
workflow   Build diagnostics #332
run        31250783124
job        93086711212
merge ref  cc480345cbcbfd4c22aecee3315cc9453429f570
```

Typecheck/lint PASS. 57/59 files and 363/369 tests passed. Five failures required the missing daily-capacity resolver; one proved a 120-minute lesson was incorrectly fitting into 60 minutes after short session history.

### Invalid semantic-fixture intermediate

```text
head       557c8029221658f7e255c62ac778f558cd9379e1
workflow   Build diagnostics #334
run        31250912972
job        93087031211
```

Three pre-existing regression fixtures unintentionally depended on default Sunday capacity. They were corrected with explicit Sunday capacity; the protected scheduling behaviors were unchanged. This run is not GREEN evidence.

### Valid GREEN

```text
head       93f9dea31e8f49c8a16c4998d7537e77e5a40aaa
workflow   Build diagnostics #336
run        31251020982
job        93087295638
merge ref  88c8a43551ed2993ad54c027a3db32ead726d196
```

59/59 files, 369/369 tests, full gate PASS.

Locked behavior:

- unfinished ordinary lessons consume `plannedDurationMinutes`;
- session history no longer shrinks ordinary placement cost;
- current day uses `todayHours`;
- explicit per-date capacity preserves zero and overrides Sunday rest;
- Sunday defaults to zero otherwise;
- ordinary weekdays use `defaultDailyHours`.

---

## 4. Task 3 — canonical full schedule projection

### Valid RED

```text
head       a1d1801056ce692447296fbb42688909257b9e72
workflow   Build diagnostics #337
run        31251139016
job        93087589080
merge ref  329649f6fad179be375e5dc48decd9be95b8741c
```

Typecheck/lint PASS. 59/60 files and 369/375 tests passed; six intended failures required the projection API.

### Valid GREEN

```text
head       787e65ec1f9af7296db5bbbafccea4c9a6bbcc71
workflow   Build diagnostics #339
run        31251305007
job        93087995567
merge ref  254a247307e8040a40f3fd55ea28cdaa42de28c8
```

60/60 files, 375/375 tests, full gate PASS.

Locked behavior:

- projection composes the existing day scheduler;
- unscheduled, unplaced-fixed and defensive-bound work are explicit blockers;
- Sunday/default/override capacity shares one resolver;
- Roadmap shifted dates equal canonical projected dates for the same fixture.

The scheduler-core projection implementation remains in `planner.ts`; `schedule-projection.ts` is the public canonical boundary. This avoids a runtime circular import.

---

## 5. Task 4 — Forecast workload, completion and confidence

### Invalid pre-RED

Build diagnostics #341 was Prettier-blocked before tests and is not behavioral RED evidence.

### Valid RED

```text
head       c3229f0dc34742bdca5613db0d2ca512e8fc509e
workflow   Build diagnostics #342
run        31251542012
job        93088566770
merge ref  ea093ae7266c4d2e663ecd4c37fc1356ab3b8375
```

Typecheck/lint PASS. 58/60 files and 374/382 tests passed. Eight intended failures covered impossible workload, lesson-level confidence, canonical completion, blocker states and capacity metadata.

### Invalid intermediates

- #345 / run `31251734964`: Prettier-blocked before tests.
- #346 / run `31251849439`: one remaining Prettier error before tests.

Neither is GREEN evidence.

### Valid GREEN

```text
head       6df802285a9c351aeb95f6e5f47294247b3ae7fc
workflow   Build diagnostics #347
run        31251931016
job        93089493552
merge ref  8c1f5e9b54476b4f137efa2ea974937952a63220
```

60/60 files, 382/382 tests, full gate PASS.

Key regression proofs:

- real-roadmap synthetic fixture = exactly 699.5 planned hours;
- eleven deterministic 120-minute completions => exactly 341 lessons / 677.5 hours remaining;
- approximately 37-minute study sessions cannot collapse workload toward 210.3 hours;
- 22 sessions across only two completed lessons remain `insufficient` confidence;
- 20 stable lesson-level ratios can reach `high`; high-variance evidence is downgraded to `medium`;
- Forecast completion equals the canonical projection last lesson date when complete;
- incomplete projection has explicit blocker state and no fake completion range;
- Forecast ViewModel no longer calls legacy arithmetic `forecast()`.

---

## 6. Task 5 — truthful Forecast presentation

### Valid RED

```text
head       d77410a28c0c821377581eaf03c11c952e18a0d0
workflow   Build diagnostics #348
run        31252067090
job        93089818566
merge ref  58b3aa0b6129f617def1d6064bf4c82cf55600de
```

Typecheck/lint PASS. 59/60 files and 383/387 tests passed; four intended presentation failures remained.

### Execution adjustment

`shiftedDates` remains an optional compatibility-only Forecast prop to avoid broad route-file churn. `ForecastCard` does not destructure or read it. Forecast correctness is separately locked by the shifted-date regression and the canonical ViewModel. Roadmap remains the active shifted-date consumer.

### Invalid intermediate

#351 / run `31252326908` / job `93090482814` was blocked by one Prettier-only line after typecheck; tests did not run.

### Pre-review GREEN

```text
head       1fbeb119d85d3552c388f7a7955ab0faa34c0428
workflow   Build diagnostics #352
run        31252411342
job        93090684473
merge ref  4235f405ed19f82c228e921e01cd447f767e933d
```

60/60 files, 387/387 tests, full gate PASS.

Production runtime tests lock:

- `Mốc học hết bài mới theo lịch hiện tại` = canonical projection date;
- planned workload remains independent from short session averages;
- unresolved work displays no fabricated date;
- many sessions on too few lessons cannot claim high confidence;
- capacity copy discloses Sunday rest and explicit date overrides;
- constant-speed and `~N phút/bài` authority are absent.

---

## 7. First Independent Review rejection and correction

### First evidence-head GREEN

```text
head       ded8e61d9c8609db1a18b9e2a392ef74437bc2f0
workflow   Build diagnostics #353
run        31252541203
job        93091006398
merge ref  c55cfbdc1df3ce030cbbbad09564d33194c9c2bd
```

60/60 files, 387/387 tests, full gate PASS.

### Rejection

Review comment: `5225689439`.

Finding: an unfinished fixed lesson with `scheduledDate < fromISO` could never be encountered by the day loop and was eventually classified as `unprojected` / `projection-bound`, contradicting fixed exact-date semantics.

### Valid correction RED

```text
head       d478bed47dd1b6dce5e4c519ec5b830eb2a3e7b4
workflow   Build diagnostics #354
run        31252693719
job        93091383056
merge ref  a9dc0785861f06166d03eb2b90d852e2dd735421
```

Typecheck/lint PASS. 59/60 files and 387/388 tests passed. Sole intended failure: a past fixed exact-date lesson was missing from `unplacedFixedLessonIds`.

### Correction GREEN

```text
head       c5c2cc25ab6e8090b4368ddcf261778cc2279b62
workflow   Build diagnostics #356
run        31252823531
job        93091702868
merge ref  4854fbcaa85f2a3fbda9967e788588135296d020
```

60/60 files, 388/388 tests, full gate PASS.

Correction proof:

- past exact-date fixed work is `unplaced-fixed`, not `projection-bound`;
- no fabricated projected date is emitted;
- it is removed from `unprojectedLessonIds`;
- Forecast consumes the corrected public projection boundary.

---

## 8. Second Independent Re-Review rejection and correction

### Refreshed evidence-head GREEN before second rejection

```text
head       e16367ae5e5957a2ee5ccbb219fba66f4e8f2938
workflow   Build diagnostics #357
run        31252949413
job        93092009426
merge ref  c44c81c01449a9ea2afa31f8123bc5f55a5d0557
```

60/60 files, 388/388 tests, full gate PASS.

### Rejection

Review comment: `5225724386`.

Finding: after the first correction, a workspace whose only unfinished work is a past fixed lesson correctly returned `unplacedFixedLessonIds`, but the Forecast ViewModel still counted that lesson as schedulable merely because `scheduledDate` was non-empty. Because the public projection performed no day loop for this definitively missed fixed work, `positiveCapacityDays === 0`, causing the primary Forecast blocker to be misreported as `no-capacity` rather than `unplaced-fixed` even when planner capacity was positive.

### Valid second correction RED

```text
head       6c2b4b0d2377070a83784f01df5689251b56d9bf
workflow   Build diagnostics #358
run        31253078967
job        93092335864
```

Typecheck/lint PASS. 60/61 test files and 388/389 tests passed. Sole intended failure:

```text
past fixed-only workspace with positive 8h capacity
expected reason: unplaced-fixed
received reason: no-capacity
```

### Second correction implementation

`selectForecastCompletion()` now excludes IDs already classified by the canonical projection as `unplacedFixedLessonIds` from the count used to decide whether projectable work has zero usable capacity. This is intentionally narrower than reordering all blocker priorities.

Consequences locked by existing and new tests:

- past fixed-only + positive capacity => `unplaced-fixed`;
- current/future fixed work that is definitively unplaced => `unplaced-fixed`;
- genuinely projectable flexible work under zero capacity => remains `no-capacity`;
- mixed blocker counts remain present in the unresolved result.

### Valid second correction GREEN / literal final source-test head

```text
head       a6326d13b438d6cc93c59925740b293711f802fb
workflow   Build diagnostics #359
run        31253203971
job        93092640783
PR merge ref at exact head  f159abef5befd12fa79dcd543996d0d71a76eae6
```

61/61 test files PASS. 389/389 tests PASS. Typecheck/lint/build/clean-tree PASS.

The new regression file `src/lib/forecast-past-fixed-regression.test.ts` proves the exact second-review finding is closed without weakening zero-capacity behavior already covered by `forecast-view-model.test.ts`.

---

## 9. Final source/test scope audit

Comparison:

```text
78f042255fe46e9bbc69193e6ef47158442cdf03
..
a6326d13b438d6cc93c59925740b293711f802fb
```

is ahead-only with no base drift. At source/test freeze, `main` remains `78f042255fe46e9bbc69193e6ef47158442cdf03`; PR #10 remains Draft/open/unmerged with that exact base.

Authorized changed paths before this final evidence refresh:

```text
docs/superpowers/plans/2026-08-08-smart-planner-p1e-hf1-forecast-correctness.md
docs/superpowers/specs/2026-08-08-smart-planner-p1e-hf1-forecast-correctness-design.md
src/components/ForecastCard.tsx
src/lib/daily-capacity.test.ts
src/lib/daily-capacity.ts
src/lib/forecast-card-runtime.test.ts
src/lib/forecast-clarity-regression.test.ts
src/lib/forecast-past-fixed-regression.test.ts
src/lib/forecast-view-model.test.ts
src/lib/forecast-view-model.ts
src/lib/planner.test.ts
src/lib/planner.ts
src/lib/planning-date-regression.test.ts
src/lib/schedule-mode-regression.test.ts
src/lib/schedule-projection.test.ts
src/lib/schedule-projection.ts
src/lib/study-duration-evidence.test.ts
src/lib/study-duration-evidence.ts
```

The two existing scheduler regression files are fixture-only adjustments needed to make their intended behavior independent from the canonical Sunday-rest rule.

No Weekly Summary file, dependency manifest, workflow, deployment configuration, persistence schema or P2 implementation file changed.

---

## 10. Known limitations / deferred work

- `studyMeta.actualMinutes` remains persisted for backward compatibility but is not Forecast authority.
- legacy `forecast()` may remain exported for compatibility; Forecast ViewModel/Card do not use it for workload/completion/confidence.
- actual/planned evidence does not automatically scale future planned workload; adaptive forecasting is deferred.
- review workload remains the existing approximate 35% informational estimate; concrete review scheduling remains scheduler-owned.
- `shiftedDates` remains compatibility-only on Forecast and cannot affect its output.
- Weekly Summary raw lesson IDs remain separate `WEEKLY-HF1 Historical lesson labels` work.
- P2 remains outside this package.

---

## 11. Final re-review handoff

Expected state after this docs-only refresh:

```text
main = 78f042255fe46e9bbc69193e6ef47158442cdf03
PR #10 = Draft / open / unmerged
base SHA = 78f042255fe46e9bbc69193e6ef47158442cdf03
literal source/test head = a6326d13b438d6cc93c59925740b293711f802fb
source/test CI #359 = GREEN
```

This evidence refresh must be exactly docs-only relative to `a6326d13b438d6cc93c59925740b293711f802fb`, followed by one natural exact-head CI run. A fresh Independent Review must inspect the resulting evidence head and must not reuse either prior rejected verdict.

Target disposition:

```text
P1E-HF1 IMPLEMENTED / REVIEW_PENDING / NOT_MERGED
```
